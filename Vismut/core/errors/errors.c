...
attribute_const VismutErrorType VismutErrorType_FromErrno(const int err,
    ...
    switch (err) {
    case ENOENT:
        return VISMUT_ERR_IO_NOT_FOUND;
    case EACCES:
    case EPERM:
        return VISMUT_ERR_IO_LOCKED;
    case EMFILE:
    case ENFILE:
        return VISMUT_ERR_IO_LIMIT;
    case ENOSPC:
    case EDQUOT:
        return VISMUT_ERR_IO_DISK_FULL;
    case EIO:
        return VISMUT_ERR_IO_FATAL;
    case EISDIR:
        return VISMUT_ERR_IO_NOT_FOUND;
    default:
        return defaultType;
...
#undef X
    ...
    if (unlikely(error > VISMUT_ERROR_COUNT || error < 0)) {
        return (const u8 *)VISMUT_ERR_UNKNOWN;
    }
